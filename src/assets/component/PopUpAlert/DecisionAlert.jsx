export default function DecisionAlert({message, setDecisionCondition, uid, handleDeleteList, setCondition}) {
      ...
      <div className="absolute w-[40%] h-[40%] bg-white flex items-center justify-center flex-col rounded-lg shadow-lg z-60">
        ...
        <h2 className="text-black text-xl font-bold font-[Montserrat] mt-2">{message}</h2>
            <div className="button-wrapper flex flex-row gap-4">
                ...
                <button className="mt-4 bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-[#6E867B] transition-colors duration-300 ease-in-out cursor-pointer" onClick={() => setDecisionCondition('')}>Cancel</button>
            </div>
      </div>
    </div>
  );
